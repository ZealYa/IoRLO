...
# 6 for MobileNet
N_IN = 6  # input dimension
N_OUT = 6  # output dimension
...
class IoRLO(gym.Env):
    ...
    def step(self, action):
        ...
        for item in action:
            ...
            if item == 1:
                number_mobile += 1
                self.state[state_pos] = pbl.mobilenet_latency_T()[state_pos] + IOF * pbe.mobilenet_energy_T()[state_pos]
            elif item == 2:
                self.state[state_pos] = pbl.mobilenet_latency_E()[state_pos]
            else: # item == 3
                self.state[state_pos] = pbl.mobilenet_latency_C()[state_pos]
            state_pos += 1
        ...
        # communication cost
        comm_cost_latency, comm_cost_energy = tool.comm_cost_mobilenet_MEC(action, N_OUT)
    ...
    def reset(self):
        self.state = pbl.mobilenet_latency_T() + IOF * pbe.mobilenet_energy_T()
